    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 px-4" onClick={onClose}>
      <div role="dialog" aria-modal="true" className="relative w-full max-w-[440px] rounded-2xl bg-white p-7 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <button
          ...
          onClick={onClose}
          aria-label="Close"
          className="absolute right-4 top-4 grid h-8 w-8 place-items-center rounded-full text-[#000d44]/60 hover:bg-black/5"
        >
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div>
            ...
            <input id="booking-name" required type="text" name="name" placeholder="Your name" className="w-full rounded-[10px] border border-black/10 px-4 py-3 text-[14px] outline-none focus:border-[#1f5fff]" />
          </div>
          <div>
            <label htmlFor="booking-email" className="mb-1.5 block text-[13px] font-semibold text-[#000d44]">Email</label>
            <input id="booking-email" required type="email" name="email" placeholder="you@example.com" className="w-full rounded-[10px] border border-black/10 px-4 py-3 text-[14px] outline-none focus:border-[#1f5fff]" />
          </div>
          <div>
            <label htmlFor="booking-phone" className="mb-1.5 block text-[13px] font-semibold text-[#000d44]">Phone Number</label>
            <input id="booking-phone" required type="tel" name="phone" placeholder="Your phone number" className="w-full rounded-[10px] border border-black/10 px-4 py-3 text-[14px] outline-none focus:border-[#1f5fff]" />
          </div>

          <button
            type="submit"
            disabled={submitting}
            className="mt-2 flex w-full items-center justify-center rounded-[10px] bg-[#1f5fff] px-7 py-3.5 text-[14px] font-semibold text-white shadow-lg shadow-[#1f5fff]/20 transition-opacity hover:opacity-90 disabled:opacity-70"
          >

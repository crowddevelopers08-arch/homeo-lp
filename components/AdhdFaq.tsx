    ...
    <section id="faq" className="bg-white px-4 py-0 font-sans sm:px-6 lg:py-10">
      <div className="mx-auto max-w-[1400px]">
        ...
        <div className="mt-12 grid items-center gap-8 lg:grid-cols-[1fr_330px_1fr] lg:gap-7 xl:grid-cols-[1fr_370px_1fr] xl:gap-10">
          <div className="grid gap-4">{faqs.slice(0,4).map((item)=><FaqCard item={item} key={item.number}/>)}</div>

          <div className="relative order-first mx-auto h-[430px] w-full max-w-[360px] lg:order-none lg:h-[540px]">
            ...
            <div className="relative h-full overflow-hidden rounded-[46%_54%_44%_56%/38%_42%_58%_62%] border-[7px] border-white shadow-[0_24px_55px_rgba(31,95,255,.2)]">
              <Image src="/adhd-faq-doctor-child.png" alt="B Homeo doctor supporting a child during a consultation" fill sizes="(max-width: 1024px) 360px, 370px" quality={95} className="object-cover"/>
            </div>
          </div>

          <div className="grid gap-4">{faqs.slice(4).map((item)=><FaqCard item={item} key={item.number}/>)}</div>
        </div>
